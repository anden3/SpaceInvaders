...
float* Buffer::Get_Pointer(int offset, int length) {
    glBindBuffer(GL_ARRAY_BUFFER, VBO);
    float* texPointer = static_cast<float*>(glMapBufferRange(
        GL_ARRAY_BUFFER,
        static_cast<unsigned long>(offset) * sizeof(float),
        static_cast<unsigned long>(length) * sizeof(float), GL_MAP_WRITE_BIT
    ));
    glBindBuffer(GL_ARRAY_BUFFER, 0);
